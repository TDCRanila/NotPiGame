...
class OBJModel; class MD2Model; class MD2Manager; class OBJManager; class ModelMatrix; 
class ShaderManager; class Shader; class CubeMap; //FW Declaration
...
public:
	...
	unsigned int createTexture(std::string); // Creates a texture and returns a pointer when finnished
	...
	unsigned int	  getTextureID(int i);	// Just returns a texture ID
	...
	std::vector<unsigned int>		textures;   // TextureIDS
